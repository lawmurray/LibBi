...
class RK43Stage {
public:
  static CUDA_FUNC_BOTH void stage1(const T1 t, const T1 h, const int p, const CX& cox, const PX& pax, T2& r1, T2& r2, T2& err) {
    ...
    X::dfdt(t, p, cox, pax, r2);
    err = e1*r2;
    r1 += a21*h*r2;
    r2 = r1 + (b1 - a21)*h*r2;
  }

  static CUDA_FUNC_BOTH void stage2(const T1 t, const T1 h, const int p, const CX& cox, const PX& pax, T2& r1, T2& r2, T2& err) {
    ...
    X::dfdt(t + c2*h, p, cox, pax, r1);
    err += e2*r1;
    r2 += a32*h*r1;
    r1 = r2 + (b2 - a32)*h*r1;
  }

  static CUDA_FUNC_BOTH void stage3(const T1 t, const T1 h, const int p, const CX& cox, const PX& pax, T2& r1, T2& r2, T2& err) {
    ...
    X::dfdt(t + c3*h, p, cox, pax, r2);
    err += e3*r2;
    r1 += a43*h*r2;
    r2 = r1 + (b3 - a43)*h*r2;
  }

  static CUDA_FUNC_BOTH void stage4(const T1 t, const T1 h, const int p, const CX& cox, const PX& pax, T2& r1, T2& r2, T2& err) {
    ...
    X::dfdt(t + c4*h, p, cox, pax, r1);
    err += e4*r1;
    r2 += a54*h*r1;
    r1 = r2 + (b4 - a54)*h*r1;
  }

  static CUDA_FUNC_BOTH void stage5(const T1 t, const T1 h, const int p, const CX& cox, const PX& pax, T2& r1, T2& r2, T2& err) {
    const T1 b5 = BI_REAL(0.34866717899928);
    const T1 c5 = BI_REAL(0.845495878172715);
    const T1 e5 = BI_REAL(0.0728532188162504); // b5 - b5hat

    X::dfdt(t + c5*h, p, cox, pax, r2);
    err += e5*r2;
    r1 += b5*h*r2;
  }
};
