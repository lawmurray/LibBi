 ...
 * $Date$
 */
#ifndef BI_CUDA_UPDATER_DYNAMICMAXLOGDENSITYVISITORGPU_CUH
#define BI_CUDA_UPDATER_DYNAMICMAXLOGDENSITYVISITORGPU_CUH
...
class DynamicMaxLogDensityVisitorGPU {
public:
  /**
   * Update.
   *
 ...
 * @internal
 *
 * Base case of DynamicMaxLogDensityVisitorGPU.
 */
template<class B, class T1, class PX, class OX>
class DynamicMaxLogDensityVisitorGPU<B,empty_typelist,T1,PX,OX> {
public:
  template<class T2>
  static CUDA_FUNC_DEVICE void accept(const T1 t1, const T1 t2, State<B,ON_DEVICE>& s, const int p, const int i,
      const PX& pax, OX& x, T2& lp) {
...
template<class B, class S, class T1, class PX, class OX>
template<class T2>
inline void bi::DynamicMaxLogDensityVisitorGPU<B,S,T1,PX,OX>::accept(const T1 t1,
  ...
  if (i < size) {
    coord_type cox;
    cox.setIndex(i);
    front::maxLogDensities(t1, t2, s, p, i, cox, pax, x, lp);
  } else {
    DynamicMaxLogDensityVisitorGPU<B,pop_front,T1,PX,OX>::accept(t1, t2, s, p,
        i - size, pax, x, lp);
  }
}
