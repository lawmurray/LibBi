...
T1 bi::ers_a_b(R& rng, const T1 a, const T1 b) {
  ...
  do {
    ...
    x = bi::log(1-x)/(-ainv) + a;
    rho = bi::exp(-0.5 * bi::pow((x - a), 2));
  } while (rng.uniform(0.0, 1.0) > rho || x > b);
  return x;
}
...
T1 bi::std_normal_dens(const T1 x) {
  return bi::exp(BI_REAL(-0.5)*bi::pow(x, BI_REAL(2.0)) - BI_REAL(BI_HALF_LOG_TWO_PI));
}

/* Uniform rejection sampling */
template<class R, class T1>
T1 bi::urs_a_b(R& rng, const T1 a, const T1 b) {
  const T1 log_phi_a = std_normal_dens(a);
  T1 x = 0.0, u = 0.0;
  ...
  do {
    x = rng.uniform(a, b);
  } while (rng.uniform(0.0, 1.0) * ub > std_normal_dens(x));
  return x;
}
...
T1 bi::lower_truncated_gaussian(R& rng, const T1 lower, const T1 mu,
  ...
  if (alpha < _t4) {
    u = mu + sigma * nrs_a_inf(rng, alpha);
  } else {
    u = mu + sigma * ers_a_inf(rng, alpha);
  }
...
T1 bi::upper_truncated_gaussian(R& rng, const T1 upper, const T1 mu,
  ...
  /* Exploit symmetry: */
  T1 u = mu - sigma * bi::lower_truncated_gaussian(rng, -beta, 0.0, 1.0);
  return u;
}
...
T1 bi::truncated_gaussian(R& rng, const T1 lower, const T1 upper, const T1 mu,
  ...
  const T1 phi_a = std_normal_dens(alpha);
  const T1 phi_b = std_normal_dens(beta);

  T1 u;
  if (alpha <= 0 && 0 <= beta) {
    if (phi_a <= _t1 || phi_b <= _t1) {
      u = mu + sigma * nrs_a_b(rng, alpha, beta);
    } else {
      u = mu + sigma * urs_a_b(rng, alpha, beta);
    }
  } else if (alpha > 0) {
    if (phi_a / phi_b <= _t2) {
      u = mu + sigma * urs_a_b(rng, alpha, beta);
      ...
      if (alpha < _t3) {
        u = mu + sigma * hnrs_a_b(rng, alpha, beta);
      } else {
        u = mu + sigma * ers_a_b(rng, alpha, beta);
      }
    }
  } else {
    if (phi_b / phi_a <= _t2) {
      u = mu - sigma * urs_a_b(rng, -beta, -alpha);
    } else {
      if (beta > -_t3) {
        u = mu - sigma * hnrs_a_b(rng, -beta, -alpha);
      } else {
        u = mu - sigma * ers_a_b(rng, -beta, -alpha);
      }
    }
  }
