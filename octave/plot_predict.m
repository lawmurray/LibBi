...
% -*- texinfo -*-
% @deftypefn {Function File} plot_predict (@var{in}, @var{invars})
%
...
% @end deftypefn
%
function plot_predict (in, invars)
    % check arguments
    if (nargin != 2)
        print_usage ();
    end
    ...
    for i = 1:length(invars)
        if (!strcmp(invars{i}, ''))
            ...
            area_between(t, Q(:,1), Q(:,3), watercolour(7, 0.5));
            plot(t, Q(:,2), 'linewidth', 3, 'color', watercolour(7));
            title(invars{i});
            plot_defaults;
        end
    end
