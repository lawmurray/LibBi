 ...
 * $Date$
 */
#ifndef BI_SSE_SSESHAREDHOSTINITVISITOR_HPP
#define BI_SSE_SSESHAREDHOSTINITVISITOR_HPP

#include "sse_shared_host.hpp"

namespace bi {
/**
 * Visitor for initialising SSE shared memory.
 *
 * @tparam B Model type.
...
class sse_shared_host_init_visitor {
public:
  /**
 ...
 * @internal
 *
 * Base case of sse_shared_host_init_visitor.
 */
template<class B, class S1>
class sse_shared_host_init_visitor<B,S1,empty_typelist> {
public:
  static void accept(State<B,ON_HOST>& s, const int p) {
    //
  }
...
template<class B, class S1, class S2>
inline void bi::sse_shared_host_init_visitor<B,S1,S2>::accept(
  ...
  subrange(*sseSharedHostState, start, size) = sse_host::fetch<B,target_type>(s, p);
  sse_shared_host_init_visitor<B,S1,pop_front>::accept(s, p);
}
