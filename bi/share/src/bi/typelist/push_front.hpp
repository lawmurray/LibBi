 ...
 * $Date$
 */
#ifndef BI_TYPELIST_PUSH_FRONT_HPP
#define BI_TYPELIST_PUSH_FRONT_HPP
...
struct push_front_impl {
  typedef typelist<TYPELIST_SCALAR,N,X,typelist<marker,reps,item,tail> > type;
};
...
struct push_front {
  typedef typename push_front_impl<T::marker,T::reps,typename T::item,typename T::tail,X,N>::type type;
};
 ...
 */
template<typelist_marker marker, int reps, class tail, class X, int N>
struct push_front_impl<marker,reps,X,tail,X,N> {
...
struct push_front<empty_typelist,X,N> {
