 ...
 */
template<class B>
class shared_host_init_visitor<B,empty_typelist> {
public:
  template<class PX>
  static void accept(PX& pax, const int p) {
    //
...
template<class B, class S>
template<class PX>
inline void bi::shared_host_init_visitor<B,S>::accept(PX& pax, const int p) {
  ...
  shared_host_init_visitor<B,pop_front>::accept(pax, p);
}
