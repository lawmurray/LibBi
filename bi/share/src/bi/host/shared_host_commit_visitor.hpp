...
class shared_host_commit_visitor<B,S1,empty_typelist> {
public:
  static void accept(State<B,ON_HOST>& s, const int p) {
    //
  }
...
template<class B, class S1, class S2>
inline void bi::shared_host_commit_visitor<B,S1,S2>::accept(
    State<B,ON_HOST>& s, const int p) {
  typedef typename front<S2>::type front;
  typedef typename pop_front<S2>::type pop_front;
  typedef typename front::target_type target_type;

  host::fetch<B,target_type>(s, p) = shared_host<S1>::template fetch<B,target_type>(s, p);
  shared_host_commit_visitor<B,S1,pop_front>::accept(s, p);
}
