...
NetCDF\index{NetCDF} files. The schema of such files are described in this
section, the schema being the names of the dimensions and variables that are
expected in such files.
...
that of the \clientref{simulate} program. This section enumerates each schema
in turn.
...
\subsubsection{Simulation schema}
...
\subsubsection{Particle filter schema}
...
schema. The \bitt{np} dimension is now interpreted as indexing
\emph{particles} not \emph{trajectories}. The following variables are
added:
\begin{itemize}
...
\subsubsection{Simulation ``flexi'' schema}
...
\subsubsection{Particle filter ``flexi'' schema}
...
\subsubsection{Kalman filter schema}

This schema is used by the \clientref{filter} program when a Kalman filter is
chosen.

\subsubsection{Optimisation schema}

This schema is used by the \clientref{optimise} program.

\subsubsection{PMCMC schema}

This schema is used by the \clientref{sample} program when a PMCMC method is
chosen.

\subsubsection{SMC$^2$ schema}
