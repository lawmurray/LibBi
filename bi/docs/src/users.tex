...
of 365 days (\bitt{-T 365}), outputting the results every day (\bitt{-K 365})
to the NetCDF\index{NetCDF} file \bitt{results/simulate.nc}.
...
intended to be modified. The most important file is the \bitt{MANIFEST} file,
...
x <- \textsl{name}(\textsl{arguments}, ...)
\end{bicode}
where \bitt{x} is some variable, referred to as the \emph{target}.
...
(e.g.  \actionref{gaussian}, \actionref{gamma} and \actionref{uniform}).
...
vector\index{vector} or matrix\index{matrix} expressions, or may require more
...
name \bitt{i} is an \emph{index}, considered declared on the left with local
scope to the action, so that it may be used on the right. The name \bitt{i} is
...
\emph{blocks}. A good example is the \actionref{ode} action. This describes a
single ordinary differential
equation\index{ordinary\,differential\,equations}. For a system with more than
one such equation, this must be grouped with the actions that describe those
other equations. The grouping is achieved with the \blockref{ode} block.
...
exist between actions, the \bitt{do..then} clause may be used to serialise
...
\}
\end{bicode}
Here, evaluating the variable \bitt{x} depends on first evaluating the
variable \bitt{eps}.

Any number of \bitt{then} parts may follow the initial \bitt{do}.

\begin{tip}
The blocks in a \bitt{do..then} clause often match up to
...
Named blocks can be used in a \bitt{do..then} clause by giving the block name
immediately after the \bitt{do} or \bitt{then} keyword. For example:
\begin{bicode}
...
Data\index{data} is input to\index{I/O} LibBi using NetCDF\index{NetCDF}
...
The syntax \bitt{x[m,n]} is used to refer to a variable in a NetCDF file named
\bitt{x}, defined across the dimensions named \bitt{m} and \bitt{n}. Note
...
\kwref{obs} variable that is not necessarily observed at all time steps. An
example of the latter is, again in a discrete-time model, having a vector
\kwref{obs} variable for which not all elements are necessarily observed at
all time steps.
...
\begin{description}
\item[\clientref{simulate}] for simulating trajectories from a model,
...
\item[\clientref{sample}] for parameter sampling problems using the model and
  observations,
...
that of the \clientref{simulate} program. This section outlines each schema.
...
This schema used by the \clientref{simulate} program. It consists of
dimensions:
\begin{itemize}
...
This schema is used by the \clientref{filter} program when a particle filter,
...
\begin{itemize}
\item \bitt{logweight[nr,np]} giving the log-weights of each particle at each
  time,
\item \bitt{ancestor[nr,np]} giving the ancestry vector at each time, and
\item \bitt{resample[nr]} indicating whether or not resampling was performed
  at each time.
\end{itemize}
...
dimensions:
\begin{itemize}
\item \bitt{np} indexing trajectories,
\item \bitt{nrp} indexing both time and trajectories, and
\item for each dimension \bitt{\emph{n}} in the model, a dimension
  \bitt{\emph{n}}.
\end{itemize}
And variables:
...
\item \bitt{start[nr]} giving, for each time, the starting index along the
  \bitt{nrp} dimension for particles associated with it,
\item \bitt{len[nr]} giving, for each time, the number of particles associated
  with it,
\item for each \kwref{param} variable \bitt{\emph{theta}} in the model,
  defined over dimensions \bitt{\emph{m},...,\emph{n}}, a variable
  \bitt{\emph{theta}[\emph{m},...,\emph{n}]}, and
...
\bitt{\emph{t}}, read \bitt{len[\emph{t}]} entries along the \bitt{nrp}
dimension, beginning at index \bitt{start[\emph{t}]}.
\end{tip}

\subsection{Particle filter ``flexi'' schema}

This scheme is used by the \clientref{filter} program when an adaptive
particle filter is chosen. It extends the simulation ``flexi'' schema. The
following variables are added:
\begin{itemize}
\item \bitt{logweight[nrp]} giving the log-weights of each particle at each
  time,
\item \bitt{ancestor[nrp]} giving the ancestry vector at each time, and
\item \bitt{resample[nr]} indicating whether or not resampling was performed
  at each time.
\end{itemize}
...
This schema is used by the \clientref{filter} program when a Kalman filter is
...
\begin{itemize}
\item \bitt{nxcol} indexing columns of covariance matrices, and
\item \bitt{nxrow} indexing rows of covariance matrices.
\end{itemize}

The following variables are added:
\begin{itemize}
\item \bitt{S\_[nr,nxcol,nxrow]} containing the upper-triangular Cholesky
...
This schema is used by the \clientref{optimise} program. It consists of
dimensions:
\begin{itemize}
...
\item for each \kwref{param} variable \bitt{\emph{theta}} in the model,
  defined over dimensions \bitt{\emph{m},...,\emph{n}}, a variable
  \bitt{\emph{theta}[ns,\emph{m},...,\emph{n}]},
\item \bitt{optimiser.value[ns]} giving the value of the function being
  optimised at each iteration, and
\item \bitt{optimiser.size[ns]} giving the value of the convergence criterion
  at each iteration.
\end{itemize}

\subsection{PMCMC schema}

This schema is used by the \clientref{sample} program when a PMCMC method is
chosen. It extends the simulation schema, with the following changes:
\begin{itemize}
...
This schema is used by the \clientref{sample} program when an SMC$^2$ method
is chosen. It extends the simulation schema, with the following changes:
\begin{itemize}
\item the \bitt{np} dimension indexes samples, not trajectories, and
\item for each \kwref{param} variable \bitt{\emph{theta}} in the model,
  defined over dimensions \bitt{\emph{m},...,\emph{n}}, there is a variable
  \bitt{\emph{theta}[nr,\emph{m},...,\emph{n},np]} instead of
  \bitt{\emph{theta}[\emph{m},...,\emph{n}]} (i.e. \kwref{param} variables are
  defined over the \bitt{nr} and \bitt{np} dimensions also).
\end{itemize}

The following variables are added:
\begin{itemize}
\item \bitt{logweight[nr,np]} giving the log-weight of each $\theta$-particle
  at each time,
\item \bitt{numberx[nr,np]} giving the number of $x$-particles associated with
  each $\theta$-particle at each time,
\item \bitt{evidence[nr]} giving the incremental evidence at each time,
\item \bitt{ess[nr]} giving the effective sample size of the $\theta$-particle
  sample at each time, and
\item \bitt{acceptancerate[nr]} giving the acceptance rate of the PMCMC
