...
\seealso{See also \secref{const}{\bitt{const}}}
...
\seealso{See also \secref{inline}{\bitt{inline}}}
...
\seealso{See also \secref{dim}{\bitt{dim}}}
...
3-dimensional spatial model may declare dimensions \bitt{x}, \bitt{y} and
\bitt{z}.
...
\seealso{See also \secref{var}{\bitt{input}, \bitt{noise}, \bitt{obs},
    \bitt{param} and \bitt{state}}} Variables are named and
...
\begin{description}
\item[\bitt{input}] for an observed variable that is set (and possibly changes
  over time) according to input from a file,
\item[\bitt{param}] for a latent variable that does not change over time,
\item[\bitt{state}] for a latent variable that changes over time,
\item[\bitt{noise}] for a latent variable that changes in time with no
  autocorrelation in time,
\item[\bitt{obs}] for an observed variable that changes over time.
\end{description}
...
A vector, matrix or higher-dimensional variable is declared by listing the
names of the \hyperref{Dimensions}{Dimensions} over which it should extend in
a comma-separated list enclosed in square brackets, after the variable
name. For example:
\begin{bicode}
dim m(50)
dim n(20)
...
state x[m,n]
\end{bicode}
declares a state variable \bitt{x}, which is a matrix of size $50 \times 20$
declared over the dimensions \bitt{m} and \bitt{n}.
...
Data is input to and output from LibBi using NetCDF\index{NetCDF} files. The
...
\begin{itemize}
\item initialisation files, containing the initial values of \bitt{state}
  variables,
\item input files, containing the values of \bitt{input} variables, possibly
  changing across time, and
\item observation files, containing the observed values of \bitt{obs}
  variables.
\end{itemize}
...
having a scalar \bitt{obs} variable that is not necessarily observed at all
time steps. An example of the latter is, again in a discrete-time model,
having a vector \bitt{obs} variable for which not all elements are necessarily
...
\item Optionally, a dimension named \bitt{np}, used to index multiple
  ...
  trajectories. Variables of type \bitt{param}, \bitt{input} and \bitt{obs}
...
\end{enumerate}
The latter dimension becomes a \emph{time dimension}\index{time
  dimension}. The time variable gives the time associated with each index of
...
Assume that we have an \bitt{obs} variable named \bitt{y}, and we wish to
...
Assume that we have an \bitt{obs} variable named \bitt{y}, which is a vector
...
\subsubsection{Coordinate variables\index{Coordinate_variables}}
...
\end{enumerate}
The second dimension becomes a \emph{coordinate dimension}\index{coordinate
  dimension}, which may also be a time dimension. Other variables defined
...
\item \marginlabel{Multicore} \index{multithreading}Experiment with the
...
\section{Style guide\label{sec:Style_guide}}\index{style guide}
