...
\blockref{proposal_parameter} block of the model specification. In this block
...
\begin{enumerate}
\item Start with an empty \blockref{proposal_parameter} block, the simulation
  time (\bitt{--end-time}) set to the time of the first observation, and the
  number of particles (\bitt{--nparticles}) set to a modest number. When
  ...
  that random number seed can be fixed (\bitt{--seed}) if you wish.
\item Steadily extend the simulation time (\bitt{--end-time}) to include a few
  more observations at a time, and increase (\bitt{--nparticles}) as needed to
  ...
  \secref{Performance guide}{Performance guide} to improve execution times and
...
\item If this fails to find a working combination with a healthy acceptance
  ...
  the name of that file to LibBi with the \bitt{--init-file} command-line
  option.
\end{enumerate}

Be aware that LibBi uses methods that are still being actively researched and
